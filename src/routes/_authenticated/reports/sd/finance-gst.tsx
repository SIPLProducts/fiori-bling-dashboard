...
const DEF = SD_REPORTS.find((r) => r.key === "finance-gst")!;

export const Route = createFileRoute("/_authenticated/reports/sd/finance-gst")({
  head: () => {
  ...
  component: FinanceGstPage,
});

function FinanceGstPage() {
  const { data: launchpad, isLoading } = useLaunchpad();
  const allowed = canAccessSdReport("finance-gst", launchpad?.screens);
