...
export function NetSalesLaunchCard({ fallback }: { fallback: React.ReactNode }) {
  ...
  if (isLoading) {
    return <div className="h-[292px] w-full animate-pulse rounded-2xl border border-border bg-launchpad-tile" />;
  }
  ...
  return (
    ...
    >
      <section
        className="relative flex h-full min-h-[292px] w-full cursor-pointer flex-col overflow-hidden rounded-2xl border p-5 text-left shadow-launchpad-tile transition-all duration-200 group-hover:-translate-y-1 group-hover:scale-[1.006] group-hover:shadow-tile-hover motion-reduce:transform-none"
        style={{
      ...
      >
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0">
            ...
            <p className="tabular mt-1 text-[30px] leading-none font-semibold" style={{ color }}>
              ₹{compact(data.total)}
            </p>
          </div>
          <span
